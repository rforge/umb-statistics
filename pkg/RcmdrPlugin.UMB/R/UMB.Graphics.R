...
plotByGroups <- function(x.name, y.name, z.name, lineType, axisLabel, legend, col, cex=1, ...){
	...
	xRange <- eval(parse(text=paste("range(", x.name, ")", sep="")))
	yRange <- eval(parse(text=paste("range(", y.name, ")", sep="")))
    top <- 3.5 + length(levs)
	if(legend){
		mar <- par("mar")
	...
	if(lineType=="p"){
		x1 <- eval(parse(text=paste("order(", x.name, "[", z.name, "=='", levs[1], "'])", sep="")))
		eval(parse(text=paste("plot(", x.name, "[", z.name, "=='", levs[1], "'][x1]", ", ", y.name, "[", z.name, "=='", levs[1], "'][x1]", ", type='p', col=col[1], cex=cex, xlab='", xLabel, "', ylab='", axisLabel, "', xlim=c(",xRange[1], ",", xRange[2],"), ylim=c(",yRange[1], ",", yRange[2],"), ...)", sep="")))
		for(i in 2:length(levs)){
			x1 <- eval(parse(text=paste("order(", x.name, "[", z.name, "=='", levs[i], "'])", sep="")))
			eval(parse(text=paste("points(", x.name, "[", z.name, "=='", levs[i], "'][x1]", ", ", y.name, "[", z.name, "=='", levs[i], "'][x1], col=col[", i,"], cex=cex, pch=",i,")", sep="")))
		}
	}
	if(lineType=="l" || lineType=="b"){
		x1 <- eval(parse(text=paste("order(", x.name, "[", z.name, "=='", levs[1], "'])", sep="")))
		eval(parse(text=paste("plot(", x.name, "[", z.name, "=='", levs[1], "'][x1]", ", ", y.name, "[", z.name, "=='", levs[1], "'][x1]", ", type='l', col=col[1], cex=cex, xlab='", xLabel, "', ylab='", axisLabel, "', xlim=c(",xRange[1], ",", xRange[2],"), ylim=c(",yRange[1], ",", yRange[2],"), ...)", sep="")))
		for(i in 2:length(levs)){
			x1 <- eval(parse(text=paste("order(", x.name, "[", z.name, "=='", levs[i], "'])", sep="")))
			eval(parse(text=paste("lines(", x.name, "[", z.name, "=='", levs[i], "'][x1]", ", ", y.name, "[", z.name, "=='", levs[i], "'][x1], col=col[", i,"] cex=cex,)", sep="")))
		}
	}
	if(lineType=="b"){
...
#		pairwiseCheckBox <- tkcheckbutton(optionsFrame, variable=pairwiseVariable)
	onOK <- function(){
		...
		closeDialog()
		if (length(labels) == 0){
			errorCondition(recall=pieChartUMB, message=gettextRcmdr("You must select a variable for labels."))
			return()
		}
		if (length(sizes) == 0){
			errorCondition(recall=pieChartUMB, message=gettextRcmdr("You must select a variable for sizes."))
			return()
		}
...
binomialDistributionPlotUMB <- function(){discreteDistributionPlotUMB("binomial")}
PoissonDistributionPlotUMB <- function(){discreteDistributionPlotUMB("Poisson")}
geomDistributionPlotUMB  <- function(){discreteDistributionPlotUMB("geom")}
hyperDistributionPlotUMB  <- function(){discreteDistributionPlotUMB("hyper")}
negbinomialDistributionPlotUMB  <- function(){discreteDistributionPlotUMB("negbinomial")}
discreteDistributionPlotUMB <- function(nameVar){
