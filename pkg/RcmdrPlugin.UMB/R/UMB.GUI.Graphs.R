...
linePlotUMB <- function(){
  ...
  onOK <- function(){
    ...
    closeDialog()
    if (0 == length(x)) {
      errorCondition(recall=linePlotUMB, message=gettextRcmdr("No x variable selected."))
      return()
    }
    if (0 == length(y)) {
      errorCondition(recall=linePlotUMB, message=gettextRcmdr("No y variables selected."))
      return()
    }
    if (1 < length(y) && length(z) == 1) {
      errorCondition(recall=linePlotUMB, message=gettextRcmdr("Only one y variable can be plotted with groups."))
      return()
    }
    if (is.element(x, y)) {
      errorCondition(recall=linePlotUMB, message=gettextRcmdr("x and y variables must be different."))
      return()
    }
    .activeDataSet <- ActiveDataSet()
...
boxPlotUMB <- function () {
	...
	onOK <- function() {
		...
		closeDialog()
		if (length(x) == 0) {
			errorCondition(recall = boxPlotUMB, message = gettextRcmdr("You must select a variable"))
			return()
		}
		.activeDataSet <- ActiveDataSet()
	...
	groupsBox(boxPlot, initialGroup=initial.group, 
			initialLabel=if (is.null(initial.group)) gettextRcmdr("Plot by groups") else paste(gettextRcmdr("Plot by:"), initial.group))
	OKCancelHelp(helpSubject = "boxplot", reset = "boxPlotUMB")
	tkgrid(getFrame(xBox), sticky = "nw")
