...
enterTableUMB <- function(){
  ...
  onOK <- function(){
    ...
    if (length(counts) != nrows*ncols){
      errorCondition(recall=enterTableUMB, message=sprintf(gettextRcmdr("Number of valid entries (%d)\nnot equal to number of rows (%d) * number of columns (%d)."), length(counts), nrows, ncols))
      return()
    }
    if (length(unique(row.names)) != nrows){
      errorCondition(recall=enterTableUMB, message=gettextRcmdr("Row names are not unique."))
      return()
    }
    if (length(unique(col.names)) != ncols){
      errorCondition(recall=enterTableUMB, message=gettextRcmdr("Column names are not unique."))
      return()
    }
...
twoWayTableUMB <- function(){
  ...
  onOK <- function(){
    ...
    if (length(count) == 0 || length(row) == 0 || length(column) == 0){
      errorCondition(recall=twoWayTableUMB, message=gettextRcmdr("You must select three variables."))
      return()
    }
    if (row == column) {
      errorCondition(recall=twoWayTableUMB, message=gettextRcmdr("Row and column variables are the same."))
      return()
    }
    percents <- as.character(tclvalue(percentsVariable))
...
linearModelUMB <- function(){
  ...
  onOK <- function(){
    ...
    closeDialog()
    if (!is.valid.name(modelValue)){
      errorCondition(recall=linearModelUMB, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue), model=TRUE)
      return()
    }
    subset <- tclvalue(subsetVariable)
	putDialog ("linearModelUMB", list (initial.contr = tclvalue(contrVariable), initial.output = c(tclvalue(regVariable),
...
#    if(tclvalue(mixVariable)   == gettextRcmdr("1")) mixed.modelGUI()
  ...
  OKCancelHelp(helpSubject="linearModel", model=TRUE, reset="resetLinearModelUMB")
  tkgrid(labelRcmdr(modelFrame, text=gettextRcmdr("Enter name for model:")), model, sticky="w")
...
resetLinearModelUMB <- function(){
	putRcmdr("reset.model", TRUE)
	putDialog("linearModelUMB", NULL)
	linearModelUMB()
}
...
################################
# Customized GLM
generalizedLinearModelUMB <- function(){
  defaults <- list(initial.contr = "contr.treatment", initial.weights="<none>", initial.offset="<none>")
  dialog.values <- getDialog("generalizedLinearModelUMB", defaults)
  ...
  onOK <- function(){
    check.empty <- gsub(" ", "", tclvalue(lhsVariable))
    if ("" == check.empty) {
      errorCondition(recall=generalizedLinearModelUMB, model=TRUE, message=gettextRcmdr("Left-hand side of model empty."))
      return()
    }
    check.empty <- gsub(" ", "", tclvalue(rhsVariable))
    if ("" == check.empty) {
      errorCondition(recall=generalizedLinearModelUMB, model=TRUE, message=gettextRcmdr("Right-hand side of model empty."))
      return()
    }
    modelValue <- trim.blanks(tclvalue(modelName))
    if (!is.valid.name(modelValue)){
      errorCondition(recall=generalizedLinearModelUMB, model=TRUE, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue))
      return()
    }
	...
	putDialog ("generalizedLinearModelUMB", list (initial.contr = tclvalue(contrVariable), initial.offset = offsetValue,
...
#    assign(modelValue, justDoIt(command), envir=.GlobalEnv)
  ...
  OKCancelHelp(helpSubject="generalizedLinearModel", model=TRUE, reset="resetGLMUMB")
...
resetGLMUMB <- function(){
	putRcmdr("reset.model", TRUE)
	putDialog("generalizedLinearModelUMB", NULL)
	generalizedLinearModelUMB()
}
...
multinomialLogitModelUMB <- function(){
  ...
  onOK <- function(){
    ...
    closeDialog()
    if (!is.valid.name(modelValue)){
      errorCondition(recall=multinomialLogitModelUMB, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue), model=TRUE)
      return()
    }
    subset <- tclvalue(subsetVariable)
    if (trim.blanks(subset) == gettextRcmdr("<all valid cases>") || trim.blanks(subset) == ""){
    ...
    check.empty <- gsub(" ", "", tclvalue(lhsVariable))
    if ("" == check.empty) {
      errorCondition(recall=multinomialLogitModelUMB, message=gettextRcmdr("Left-hand side of model empty."), model=TRUE)
      return()
    }
    check.empty <- gsub(" ", "", tclvalue(rhsVariable))
    if ("" == check.empty) {
      errorCondition(recall=multinomialLogitModelUMB, message=gettextRcmdr("Right-hand side of model empty."), model=TRUE)
      return()
    }
    if (!is.factor(eval(parse(text=tclvalue(lhsVariable)), envir=get(.activeDataSet, envir=.GlobalEnv)))){
      ...
      errorCondition(recall=multinomialLogitModelUMB, message=gettextRcmdr("Response variable must be a factor"))
      return()
      }
    if (baseValue!="" && !is.element(baseValue, justDoIt(paste("levels(", ActiveDataSet(), "$", tclvalue(lhsVariable), ")", sep="")))){
      errorCondition(recall=multinomialLogitModelUMB, message=gettextRcmdr("'Base level' must be a level used in the response."))
      return()
    }
    if (is.element(modelValue, listMultinomialLogitModels())) {
      if ("no" == tclvalue(checkReplace(modelValue, type=gettextRcmdr("Model")))){
        UpdateModelNumber(-1)
        multinomialLogitModelUMB()
        return()
      }
    }
    if(baseValue != ""){
...
ordinalRegressionModelUMB <- function(){
  ...
  onOK <- function(){
    ...
    closeDialog()
    if (!is.valid.name(modelValue)){
      errorCondition(recall=ordinalRegressionModelUMB, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue), model=TRUE)
      return()
    }
    subset <- tclvalue(subsetVariable)
    if (trim.blanks(subset) == gettextRcmdr("<all valid cases>") || trim.blanks(subset) == ""){
    ...
    check.empty <- gsub(" ", "", tclvalue(lhsVariable))
    if ("" == check.empty) {
      errorCondition(recall=ordinalRegressionModelUMB, message=gettextRcmdr("Left-hand side of model empty."), model=TRUE)
      return()
    }
    check.empty <- gsub(" ", "", tclvalue(rhsVariable))
    if ("" == check.empty) {
      errorCondition(recall=ordinalRegressionModelUMB, message=gettextRcmdr("Right-hand side of model empty."), model=TRUE)
      return()
    }
    if (!is.factor(eval(parse(text=tclvalue(lhsVariable)), envir=get(.activeDataSet, envir=.GlobalEnv)))){
      #        if (!is.factor(eval(parse(text=tclvalue(lhsVariable)), envir=eval(parse(text=.activeDataSet), envir=.GlobalEnv)))){
      errorCondition(recall=ordinalRegressionModelUMB, message=gettextRcmdr("Response variable must be a factor"))
      return()
      }
    if (is.element(modelValue, listProportionalOddsModels())) {
      if ("no" == tclvalue(checkReplace(modelValue, type=gettextRcmdr("Model")))){
        UpdateModelNumber(-1)
        ordinalRegressionModelUMB()
        return()
      }
    }
    formula <- paste(tclvalue(lhsVariable), tclvalue(rhsVariable), sep=" ~ ")
...
numericalSummariesUMB <- function(){
  defaults <- list(initial.x=NULL, initial.mean="1", initial.median="0", initial.sum="0", initial.sumSq="0", initial.sd="1", initial.sdErr="0", initial.var="0", initial.cv="0",
                   ...
                   initial.group=NULL)
  dialog.values <- getDialog("numericalSummariesUMB", defaults)
  ...
  groupsBox(recall=numericalSummariesUMB, label=gettextRcmdr("Summarize by:"), 
  ...
  onOK <- function(){
    ...
    putDialog("numericalSummariesUMB", list(
    ...
    if (length(x) == 0){
      errorCondition(recall=numericalSummariesUMB, message=gettextRcmdr("You must select a variable."))
      return()
    }
    closeDialog()
    ...
    stats <- paste("c(",
                   paste(c('"mean"', '"median"', '"sum"', '"sumSq"', '"sd"', '"sdErr"', '"var"', '"quantiles"', '"cv"', '"skewness"', '"kurtosis"')
                         ...
                         collapse=", "), ")", sep="")
    if (stats == "c()"){
      errorCondition(recall=numericalSummariesUMB, message=gettextRcmdr("No statistics selected."))
      return()
    }
    ...
    command <- if (.groups != FALSE) {
      ...
      paste("numSummaryUMB(", vars, ", groups=", grps, ", statistics=", stats, 
            ", quantiles=", quants, type.text, ")", sep="")
    }
    else  paste("numSummaryUMB(", vars, ", statistics=", stats, 
                ", quantiles=", quants, type.text, ")", sep="")
    doItAndPrint(command) 
    tkfocus(CommanderWindow())
  }
  OKCancelHelp(helpSubject="numSummary", reset="numericalSummariesUMB")
  tkgrid(getFrame(xBox), sticky="nw")    
...
independentSamplesTTestUMB <- function(){
...
independentSamplesZTestUMB <- function(){
...
#      assign(modelValue, justDoIt(command), envir=.GlobalEnv)
  ...
  radioButtonsUMB(formatFrame,name="linear", buttons=c("linear", "quadratic"), values=c("linear", "quadratic"), initialValue = "linear",
